...
The rest of the workflow is visualised below. Essentially, it is a series of steps where two individuals are developing our workflow in collaboration. At each step edits to the workflow are made offline (in RStudio on local laptops) and then synced back to GitHub (online). There is the OWNER of the _main_ branch of the git repository and the COLLABORATOR, who is contributing to the _main_ branch by creating their own _fork_ of the repository, which they edit and and then make _pull requests_ to the OWNER to have their modifications _merged_ back into the _main_. Moving changes between either laptop and the online repositories requires _pulling_ the latest changes from the online repository to the laptop or _committing_ (naming a version) and _pushing_ changes from the laptop to online. Ultimately, it is the _main_ branch that we hope to have up to date, whereas the _fork_ can usually be discarded once all proposed changes (pull requests) are _merged_.
...
We begin with each of you creating a repository for which you are the "Owner" and your partner will be the "Collaborator".

1.  Go to your account on [github.com](https://github.com/) and under the Repositories tab click on the "New" button (green with a picture of a book on it, or click the plus sign in the top-right, then "New Repository")
...
3.  DO NOT click the "Initialize this repository with a README" checkbox. We're going to add a README later
4.  Optionally, add a licence (e.g. MIT), and add R to the .gitignore (check ".gitignore" and search for the *R* template)
5.  Click "Create Repository"
6.  Copy the URL of your new repository

> WARNING!!! This is where most mistakes creep in! Always make sure you are using the URL from ***your*** GitHub account and not your partner's. You should never be cloning their repository to your laptop or trying to edit it online. You only work on your copies and send "pull requests" when yo want them to merge your changes to their version.

7.  To clone your repository to your computer, open RStudio so we can create a New Project using your URL. **Note:** If you already have a project open it will close when you do so. Don't worry, you can return to that project after the prac using the drop-down in the top-right of the RStudio window.

-   Select `New Project` from the drop-down menu in the top right corner
-   Select `Version Control` then `Git`
-   Paste in ***your*** URL
-   Select where on your laptop you want the folder to be kept. I keep mine as close to the root directory as possible and avoid having it in a folder that syncs online automatically (e.g. with Dropbox, OneDrive or Google Drive) as this can create problems.
...
Within this project we'll create separate files for each function in the analysis and then call them from a master script (or makefile) at the end. To make the order of the workflow clear, and call them from the makefile, we'll want to name the files systematically. We'll also want to keep the functions in a folder called "functions" to keep things neat. In the first file we'll implement the download.NDVI() function.

> NOTE: You'll see that I've provided the R code for how I actually accessed the data using library("MODISTools"), but have left that commented out. The function can take quite a long time so I thought best that we just read the output of that call from a file I've saved to my GitHub repository.
...
1.  In RStudio, in the "Files" tab (usually in the bottom-right window), click "Create a new folder" and name it "functions"
2.  In RStudio, click `File` \> `New File` \> `R Script`
3.  Copy and Paste the above function into this file 
    - Note: There's a copy button in the top right corner of the grey code block

4.  Save the file as "01_download.NDVI.R" in the "functions" folder in the directory for the R project
    - Note: Using the exact file and folder names is important, because we'll be calling it from other code later

5.  From the "Git" tab (top-right window in RStudio), click the box next to the file you just created. This is equivalent to *git add* if you were doing this using command line in bash or terminal
...
3.  Copy the URL ***of your own version*** of the repo and follow the instructions above for cloning ***your copy of the repository*** to your local machine in RStudio.
4.  Open a new file, enter the code below, and then save the file as "02_plot.NDVI.R" in the "functions" folder in the directory for the R project
...
1.  Once the COLLABORATOR has created the pull request, you should get an automatic email and also be able to see the pull request under the `Pull Requests" tab on your Github page for the project (refresh the webpage if not).
...
2.  Next, open up a new R file, add the code below for fitting the models with MLE, and save as "03_negexp.R" in the "functions" folder in the directory for the R project
...
To use MLE we need to define a Likelihood, which is the relationship between the value of the parameter and the probability of some observed data. In this case we're assuming a Normal likelihood (hence the use of `dnorm()` in the function). This is the `data model` that we discussed in lectures. I've given the likelihood a standard deviation (0.001) that I've made up to represent the uncertainty (ideally this should come from the data; e.g. the atmospheric and other corrections that were used to estimate the "surface reflectance" MODIS product from the "top of atmosphere reflectance" that is actually observed by the satellite). In a more detailed analysis we'd want to follow up to check both these assumptions, but it's a simple starting point for this practical demonstration.
...
First are the `process models` themselves, *pred.negexp()* and *pred.negexpS()*, which translate the equations:
...
## Task 5: Collaborator adds the master script, Owner runs the master script
...
3.  Navigate to <https://github.com/jslingsby/BIO3019S_Ecoforecasting/blob/master/16-pracoutput.Rmd>, copy the raw contents
4.  Create a new Rmarkdown file in RStudio, add your name, and paste the contents copied from the document at the link above.
5.  In the header at the top of this document, find the line that says `output: html_document` and change this to `output: rmarkdown::github_document`
6.  Save this file as "README.Rmd" in the root directory for the R project (i.e. NOT in the "functions" folder).
7.  Within RStudio's Git tab, add the file and Commit. Use the Push (up arrow) button to push this to your own repository
8.  On Github.com, submit a pull request as before
...
3.  Open "README.Rmd" (Do NOT answer the questions in this file) 

4.  Click `Knit` (top-centre of script window in RStudio) to generate an md or html document (it should pop up).
  - If the file won't knit properly, it suggests you've done something wrong in the workflow... You need to fix this.

5.  Once complete, commit "README.Rmd" and "README.md" and push them back to the repo. 

6.  Finally, answer the questions in a separate document and indicate your name, your partner's name, the links to both your repository and your partner's (making clear which is which), and upload the document under the assignment I created on Amathuba. 

## Marking

Your mark will be based on your answers to the questions, whether you completed the collaborative activity correctly (I can check because of GitHub's version control), and your answers to the quiz on the reading.

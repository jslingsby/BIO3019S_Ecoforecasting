...
#| label: ecological_predictability
#| fig-cap: "The predictability of a forecast is measured by (1) the rate at which forecast uncertainty (blue shaded area) grows into the future and (2) the limit at which the forecast performs no better than a historical baseline. The growth in uncertainty is controlled by uncertainty in (and sensitivity to) the initial conditions (IC), exogenous drivers (X), parameter uncertainty ($\\theta$), parameter variability ($\\alpha$) and model structural (process) errors ($\\sigma$). From @Dietze2024."
...
1) the rate at which uncertainty grows into the future (i.e. loss of proficiency), 

2) the limit at which the forecast performs no better than a historical baseline (i.e. model adds no value)
...
Dietze [@Dietze2017;@Dietze2017b] express the sources of prediction uncertainty (see "Growth rate" in previous Figure) as an equation (here spread over multiple lines): 
...
2. exploring the relative contribution of sources of uncertainty to overall uncertainty in streamflow losses - to ***guide efforts to improve future estimates***

3. providing ***a repeatable workflow***[^1] to make it easy for anyone to query our methods or recalculate estimates as and when updated data become available

[^1]: [In truth, it is no longer repeatable because it used Google Fusion Tables which no longer exists... Which reinforces that one should always use open software!!!]{.fragment fragment-index=1}
...
3. Within each catchment, for each pixel we:
    a. Estimate pixel-level naturalized runoff$^1$ by sampling precipitation and converting to runoff

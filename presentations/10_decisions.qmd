...
![The procedure of particle filtering from @Kim2018. Here, values are sampled from the posterior at time $k-1$ to be the prior at time $k$. These are then weighted by the likelihood (i.e. the new data) to generate a new posterior, which is then resampled to provide the prior in time $k+1$.](img/particlefilter.jpg){width=80%}

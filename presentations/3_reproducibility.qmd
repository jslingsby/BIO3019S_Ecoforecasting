...
!["'Is there a reproducibility crisis?' <br> A survey of \>1500 top scientists [@Baker2016; @Penny2016].](img/reprocrisis-1.png)
...
-   **Full replication is a huge challenge** (and sometimes impossible)
-   **Some studies may not be entirely replicable** (e.g. rare phenomena, long term records, very expensive projects like space missions).
-   Where the "gold standard" of full replication cannot be achieved, we have to settle for a lower rung on **The Reproducibility Spectrum** [@Peng2011]
...
This entails overlapping/intertwined components, namely:
...
Coding is communication. Messy code is bad communication, hampering collaboration and making it easier to make mistakes...
:::
...
- Scientific coding languages like Python or R allow you to **run almost any kind of analysis in one scripted workflow** 
...
- Most coding languages are **open source** (e.g. R, Python, JavaScript, etc)
    - You can share your code other users don't have to pay
    - Transparency - Lots of people have checked the background code and functions you're using, not just the software company
...
-   use **commenting** to explain what you're doing at each step or in each function
    - "notebooks" like RMarkdown or Jupyter Notebooks are handy for fulfilling roles like documentation, master/Makefiles, etc and can be developed into manuscripts
...
I've only ever used Git and [GitHub](https://github.com/), but they all have similar functions. Technical terms from GitHub appear like `this`. You can look them up [here](https://docs.github.com/en/get-started/quickstart).

The advantages of version control as I see them:
...
-   The **system is online**, but you **can also work offline** by `cloning` the repo to your local machine. You can `push` to or `pull` from the online repo to keep local and online versions updated.

## Version control continued... {.smaller background-color="#c2c190"}

-   **Changes are tracked and reversible** through `commits`
    - Changes in the contents of a repo you must by `commit`ed with a `commit message` before pushing to the online repo. Each `commit` is a recoverable `version` that can be `compared` or `reverted` to
...
This should make more sense after the practical...
...
- In R, you can simply run the `sessionInfo()` function, like so:
...
![Let's start being more specific about our miracles... Cartoon &copy; Sidney Harris. Used with permission [ScienceCartoonsPlus.com](www.ScienceCartoonsPlus.com)](img/miracle.jpg)
...
1.  Its ***transparent and open***, helping us avoid mistakes or track down errors in analyses

2.  It makes it ***easier to write papers***

3.  It ***helps the review process***

4.  It ***enables continuity*** of the research
...
-   This should be less of a problem as more tools lik contained computing environments become available

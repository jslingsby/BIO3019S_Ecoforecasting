...
2.  The concept of ***likelihood*** and the estimation of maximum likelihood
...
::: {.column width="50%"}
- If the residuals are not normally distributed, consider a different model (e.g. non-linear), or another approach!
...
- This ***tells you nothing about the probability of the parameters actually being the estimates you arrives at by minimizing the sums of squares***!!!?
...
Viewed differently, with MLE we assume we have the correct model and use MLE to choose parameter values that maximize the *conditional probability* of the data given those parameter values,  $P(Data|Parameters)$.
...
BUT! **What we really want is to know is the conditional probability of the parameters given the data** $P(Parameters|Data)$, because this allows us to ***express uncertainty in the parameter estimates as probabilities***[^7_bayesian-1]. 

  - To get there, we apply a little probability theory, providing a surprisingly useful byproduct!

[^7_bayesian-1]: Note that $likelihood \neq probability$. I don't want to spend time on the distinction, but it is a very important concept. The area under a probability distribution sums to 1, but the area under a likelihood profile curve does not. This is because probability relates to the set of possible results, which are exhaustive and mutually exclusive, whereas likelihood relates to the possible hypotheses, which are neither exhaustive nor mutually exclusive (i.e. there's no limit to the number of potential hypotheses, and hypotheses can be nested or overlapping).
...
::: {.column width="50%"}
Based on the joint probability we can define two ***conditional probabilities***:
...
This means we can derive the information we want to know, $P(Parameters|Data)$, as a function of the information maximum likelihood estimation has given us, $P(Data|Parameters)$...
...
- While $p(\theta)$ is almost an unexpected consequence of having solved the equation for $p(\theta|D)$, ***this is where the magic of Bayes lies!***
...
*"But how can we know anything about the parameter values without the data?" You ask.*
...
- it is our "context of previous knowledge or information" 

...
-   Here I provide a brief introduction to state-space models, but first I must introduce ***latent variables***, since SSMs use latent variables.
:::
:::
...
- i.e. their true value is not known and can only be inferred indirectly through a model from other variables that can be observed and measured. They typically relate to the state variable or data of interest.

. . .

- different to parameters, which are typically fixed, unknown constants that define the structure and behavior of the model, e.g. regression coefficients in a linear model. 
...
The name comes from the focus on estimating the state as a _latent_ variable.[^1]

<br/>

Explicitly separates observation errors from process errors, addressing autocorrelation and allowing attractive flexibility... 

<br/>

[^1]: also referred to as _Hidden Markov models_ ("hidden" ~ the latent variable(s); "Markov" ~ the recursive nature,  with the next state in time a function of the current)
...
The process model[^2]: $$z_{t+1}\sim\mathit{N}(z_{t},\tau_{add})$$
...
[^2]: Note that the process and data model can also be referred to as the _state_ and _observation_ models respectively.
...
$$
\underbrace{z_{t}|...}_\text{current state} \; \propto \; \underbrace{ {\mathit{N}(z_{t}|z_{t-1},\tau_{add})}}_\text{previous state} \; \underbrace{ {\mathit{N}(y_{t}|z_{t},\tau_{obs})}}_\text{current observation} \; \underbrace{ {\mathit{N}(z_{t+1}|z_{t},\tau_{add})}}_\text{next state} \;
$$
:::
::: {.column width="25%"}
![](img/statespace_augermethe_a.png)
:::
:::
...
In fact, the posterior of $z_{t}$ is proportional to the product of the three.
...
dat <- data.frame(
  ...
  state = rep(rangeP,4),
  Density = c(prev*obs*nex/3, prev, obs, nex) #fudged this... doing something wrong...?
    )
...
dat <- data.frame(
  term = factor(c(rep("current state", 100), rep("previous time", 100), rep("current observation", 100), rep("next time", 100)),
                levels = c("current state", "previous time", "current observation", "next time")),
  state = rep(rangeP,4),
  Density = c(((prev+1)*(obs+1)*(nex+1)/8), prev, obs, nex) #fudged this... doing something wrong...?
    )
...
$$
\underbrace{z_{t}|...}_\text{current state} \; \propto \; \underbrace{ {\mathit{N}(z_{t}|z_{t-1},\tau_{add})}}_\text{previous state} \; \underbrace{ {\mathit{N}(y_{t}|z_{t},\tau_{obs})}}_\text{current observation} \;
$$
...
dat <- data.frame(
  ...
  state = rep(rangeP,3),
  Density = c((prev+1)*(obs+1)/5, prev, obs) #fudged this... doing something wrong...?
    )
...
$$
\underbrace{z_{t}|...}_\text{forecast state} \; \propto \; \underbrace{ {\mathit{N}(z_{t}|z_{t-1},\tau_{add})}}_\text{previous state} \; 
$$
...
Most forecasts would have a more influential process model.
:::
::::

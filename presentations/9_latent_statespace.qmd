...
The name comes from the focus on estimating _latent_ state variables.
...
a) The dependence among states $z_{t-1}$, $z_t$, $z_{t+1}$, ... can be modelled explicitly in the process model. The dependence of the observations $y_t$ on the states $z_t$ allows observations to be assumed to be independent. 
...
For a Bayesian model this would also require priors on the process error ($\tau_{add}$), observation error ($\tau_{obs}$) initial condition of the state variable ($x_0$).

[^1]: Note that the process and data model can also be referred to as the state and observation models respectively.
...
dat <- data.frame(
  ...
  state = rep(rangeP,4),
  Density = c(prev*obs*nex/45, prev/15, obs/15, nex/15)
    )
...
Where the terms are similar, the distribution of the state estimate peaks.
...
Where the terms differ, the distribution of the state estimate flattens.
...
dat <- data.frame(
  ...
  state = rep(rangeP,3),
  Density = c(prev*obs/30, prev/15, obs/15)
    )
...
Our postfire recovery example could use a state update equation with process model[^2]:
...
The process model represents negative exponential growth, seasonality and a fire switch that tells the model to follow the exponential when fires do not occur ($z_{i,t} = 0$) resets the vegetation state to postfire greenness ($\alpha$) when they do occur ($z_{i,t} = 1$).

[^2]: from [Slingsby et al. 2023](http://dx.doi.org/10.1111/2041-210x.14046)

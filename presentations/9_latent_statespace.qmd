...
\begin{gather}
z_{t}|... \; \propto \; \underbrace{{\mathit{N}(z_{t}|z_{t-1},\tau_{add})}}_\text{previous} \; \underbrace{{\mathit{N}(y_{t}|z_{t},\tau_{obs})}}_\text{observation} \; \underbrace{{\mathit{N}(z_{t+1}|z_{t},\tau_{add})}}_\text{next} \;
\end{gather}


$$
z_{t}|... \; \propto \; \underbrace{\mathit{N}(z_{t}|z_{t1})}_\text{previous} \;
$$
...
In fact, the posterior of the current state ($z_{t}$) is just the product of the three normal distributions.

## State-space models - example {background-color="#33431e"}
...
A state-space model of our postfire recovery model example would use the following state update equation:

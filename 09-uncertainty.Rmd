...
**Question 1** is very much dependent on question 2, but more specifically it depends on the combination of the **rate of accumulation of uncertainty** (or loss of proficiency) through time, as it tends towards being no better than the historical baseline (which doesn't require a model). Together these determine the ***"forecast limit"*** (Figure \@ref(fig:ecologicalpredictability); @Dietze2024). Some forecasts may lose proficiency very quickly, crossing the forecast limit sooner. Conversely, if the forecast loses proficiency more slowly, the forecast limit may be further into the future.

<br>


```{r ecologicalpredictability, echo=F, out.width='100%', fig.align='center', fig.cap= "The predictability of a forecast is measured by (1) the rate at which forecast uncertainty (blue shaded area) grows into the future and (2) the limit at which the forecast performs no better than a historical baseline. The growth in uncertainty is controlled by uncertainty in (and sensitivity to) the initial conditions (IC), exogenous drivers (X), parameter uncertainty ($\\theta$), parameter variability ($\\alpha$) and model structural (process) errors ($\\sigma$). From @Dietze2024."}

knitr::include_graphics("img/ecological_predictability.webp")
```
...
Addressing **question 2**, *"What determines prediction uncertainty?"* involves exploring the sources that determine the "Growth rate" of uncertainty in Figure \@ref(fig:ecologicalpredictability), namely initial conditions (IC), exogenous drivers (X), parameter uncertainty ($\theta$), parameter variability ($\alpha$) and model structural (process) errors ($\sigma$).

<br>

Dietze [@Dietze2017;@Dietze2017b] expresses these sources of prediction uncertainty in his book in the form of **an equation** (note that I've spread it over multiple lines): 
